...
- A `JSON` column declaring a path both as a value and as the parent of another path, such as `JSON(a Int64, a.b Nullable(Int64))`, no longer raises `TypeError` out of the codec and abort the whole query. A deeper path holding nothing now gives way to the shallower scalar. Closes [#997](https://github.com/ClickHouse/clickhouse-connect/issues/997).
